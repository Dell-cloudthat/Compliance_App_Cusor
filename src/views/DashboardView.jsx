...
// ─── Inline Trust Score Dashboard Widget ─────────────────────────────────────
// Fetches live trust score and renders a compact card with a circular gauge.
// Clicking opens the full Trust Portal showcase page.

function TrustScoreWidget({ setActiveView }) {
  ...
  const TIER_COLORS = {
    Excellent: { ring: '#22c55e', text: 'text-green-500', bg: 'bg-green-500/10', border: 'border-green-500/20' },
    Strong:    { ring: '#3b82f6', text: 'text-blue-500',  bg: 'bg-blue-500/10',  border: 'border-blue-500/20' },
    Developing:{ ring: '#eab308', text: 'text-yellow-500',bg: 'bg-yellow-500/10',border: 'border-yellow-500/20' },
    Foundational:{ ring: '#f97316',text:'text-orange-500',bg:'bg-orange-500/10', border:'border-orange-500/20' },
    'At Risk': { ring: '#ef4444', text: 'text-red-500',   bg: 'bg-red-500/10',   border: 'border-red-500/20' },
  };

  const score  = data?.trust_score || 0;
  const tier   = data?.tier || '—';
  const col    = TIER_COLORS[tier] || TIER_COLORS['Developing'];
  const sz     = 64;
  const r      = sz / 2 - 6;
  ...
  const pillars = data?.pillars
    ? [
        { key: 'security',   label: 'Security',    score: data.pillars.security?.score   || 0, color: '#3b82f6' },
        { key: 'compliance', label: 'Compliance',  score: data.pillars.compliance?.score || 0, color: '#22c55e' },
        { key: 'ai',         label: 'AI Protect',  score: data.pillars.ai?.score         || 0, color: '#a855f7' },
        { key: 'data',       label: 'Data',        score: data.pillars.data?.score        || 0, color: '#f97316' },
      ]
    : [];

  if (loading) {
    return (
      <div className="bg-card border border-[hsl(var(--border))] rounded-xl p-4 flex items-center gap-3 animate-pulse">
  ...
  return (
    <button
      ...
      className={`w-full text-left bg-card border rounded-xl p-4 hover:opacity-90 transition-all group ${col.bg} ${col.border}`}
    >
      <div className="flex items-center gap-4">
        {/* Ring */}
        <div className="relative shrink-0" style={{ width: sz, height: sz }}>
          <svg width={sz} height={sz} className="-rotate-90">
            <circle cx={sz/2} cy={sz/2} r={r} fill="none" strokeWidth="6" className="stroke-muted/30" />
            <circle cx={sz/2} cy={sz/2} r={r} fill="none" strokeWidth="6"
              stroke={col.ring} strokeLinecap="round"
              strokeDasharray={`${(score/100)*circ} ${circ-(score/100)*circ}`} />
          </svg>
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <span className={`text-base font-black ${col.text}`}>{score}</span>
          </div>
        </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5">
            <span className="text-sm font-bold text-foreground">Tenant Trust Score</span>
            <span className={`text-xs font-semibold px-2 py-0.5 rounded-full border ${col.bg} ${col.text} ${col.border}`}>{tier}</span>
          </div>
          <div className="flex gap-3">
            {pillars.map(p => (
              <div key={p.key} className="flex items-center gap-1">
                <div className="w-1.5 h-1.5 rounded-full shrink-0" style={{ background: p.color }} />
                <span className="text-xs text-muted-foreground">{p.label} {p.score}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* CTA */}
        <div className="flex items-center gap-1.5 text-xs font-semibold text-muted-foreground group-hover:text-foreground transition-colors shrink-0">
          <Shield className="w-3.5 h-3.5" /> View Trust Portal
...
const renderDashboard = () => {
  ...
  <div className="space-y-4">
    {/* AI Assessment Wizard CTA — shown prominently at the top of the dashboard */}
    <div className="relative overflow-hidden rounded-xl bg-gradient-to-r from-primary/15 via-purple-500/10 to-blue-500/10 border border-primary/25 p-5">
      ...
      <div className="relative flex flex-col sm:flex-row sm:items-center gap-4">
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs font-semibold text-primary uppercase tracking-wide">AI-Powered · Free</span>
          </div>
          <h3 className="text-base font-bold text-foreground">
            Not sure which compliance frameworks you need?
          </h3>
          <p className="text-sm text-muted-foreground mt-0.5">
            Answer 4 short questions about your organisation and our AI engine recommends the right frameworks with a prioritised implementation roadmap.
          </p>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          <button
            onClick={() => setActiveView('wizard')}
            className="flex items-center gap-2 px-5 py-2.5 bg-primary text-primary-foreground rounded-xl hover:bg-primary/90 font-semibold text-sm shadow-md shadow-primary/20 transition-all whitespace-nowrap"
          >
    ...
    {/* Trust Score Dashboard Widget */}
    <TrustScoreWidget setActiveView={setActiveView} />
