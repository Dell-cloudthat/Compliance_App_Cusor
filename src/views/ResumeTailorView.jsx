...
export default function ResumeTailorView() {
  ...
  const exportMarkdown = useMemo(() => {
    const lines = [];
    lines.push(`# Tailored Resume Draft (Analysis)`);
    if (jobTitle || company) lines.push(`**Role**: ${jobTitle || '[Job Title]'} • **Company**: ${company || '[Company]'}`);
    lines.push('');
    lines.push(tailoring.tailoredSummary.trim());
    lines.push('');
    lines.push('## Skills (tailored)');
    lines.push(tailoring.tailoredSkills.length ? tailoring.tailoredSkills.join(' • ') : '[Add skills]');
    lines.push('');
    lines.push('## Requirement-by-requirement bullets');
    requirements.forEach((r, idx) => {
      lines.push(`- **R${idx + 1}**: ${r.text}`);
      lines.push(`  - Draft bullet: ${r.suggestion}`);
      if (r.matchedSkills?.length) lines.push(`  - Matched skills: ${r.matchedSkills.join(', ')}`);
    });
    lines.push('');
    lines.push('## Base experience (for editing)');
    lines.push(baseExperience ? baseExperience.trim() : '[Paste experience bullets here]');
    return lines.join('\n');
  }, [requirements, tailoring.tailoredSummary, tailoring.tailoredSkills, jobTitle, company, baseExperience]);

  const handleCopy = async () => {
    const payload = onePageText?.trim() ? onePageText : exportMarkdown;
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Upload your resume + job requirements (or paste). The tool extracts both, compares them requirement-by-requirement, then generates a concise one-page tailored resume you can edit.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-card border border-[hsl(var(--border))] rounded-xl p-5 space-y-4">
          ...
          <div className="flex items-center justify-between gap-3">
            <div className="text-xs text-muted-foreground">
              ATS keywords covered: <span className="text-foreground font-medium">{tailoring.atsCoveredCount}</span>
              <span className="text-muted-foreground"> / {tailoring.atsTotalCount}</span>
            </div>
            <div className="flex flex-wrap gap-2 justify-end">
              {tailoring.atsKeywords.slice(0, 8).map((k) => (
                <span
                  key={k.term}
                  className="px-2 py-1 rounded-md text-xs bg-muted/40 border border-[hsl(var(--border))] text-muted-foreground"
                  title={`Count in JD: ${k.count}`}
                >
                  {k.term}
                </span>
              ))}
        ...
        <div className="bg-card border border-[hsl(var(--border))] rounded-xl p-5 space-y-4">
          ...
          <div className="bg-muted/20 border border-[hsl(var(--border))] rounded-lg p-3">
            <div className="text-xs text-muted-foreground mb-1">Tailored summary draft</div>
            <pre className="whitespace-pre-wrap text-sm text-foreground">{tailoring.tailoredSummary.trim()}</pre>
          </div>
        </div>
      </div>
